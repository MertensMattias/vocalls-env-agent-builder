// =============================================================================
// PAYMENT ASSISTANT — CONTEXT PREPARATION (Refactored v4)
...
//   - agentContext: object for action handlers (toolData, toolDefinitions, actionResults, cdbCaseData, etc.)
//
// Notes:
...
// Tool config: description, confirmation, entities, silent.
// Keys must match Dialog node output relation names.
// confirmation: Explicit | Implicit | Auto | None (matches Vocalls Dialog Node)
// silent: true = call without user confirmation when context is clear (e.g. end_conversation)
var TOOL_CONFIG = {
    request_payment_deferral: {
        description: {
            NL: "Regelt uitstel van betaling. Roep deze functie alleen aan wanneer de gebruiker expliciet heeft bevestigd dat zij het uitstel willen.",
            FR: "Organise le report de paiement. Appelez cette fonction uniquement lorsque l'utilisateur a explicitement confirme qu'il souhaite le report.",
            DE: "Richtet Zahlungsaufschub ein. Rufen Sie diese Funktion nur auf, wenn der Nutzer explizit bestätigt hat, dass er den Aufschub möchte.",
            EN: "Arranges payment deferral. Call this function only when the user has explicitly confirmed they want the deferral."
        },
    ...
    create_payment_plan: {
        description: {
            NL: "Maakt het afbetalingsplan aan. Roep deze functie alleen aan wanneer de gebruiker expliciet heeft bevestigd dat zij het plan willen.",
            FR: "Cree le plan de paiement. Appelez cette fonction uniquement lorsque l'utilisateur a explicitement confirme qu'il souhaite le plan.",
    ...
    send_url_by_email: {
        description: {
            NL: "Stuurt e-mail met link naar de klantenzone. Roep deze functie aan wanneer de gebruiker expliciet ja zegt op de vraag of zij een e-mail willen.",
            FR: "Envoie un e-mail avec le lien vers la zone client. Appelez cette fonction lorsque l'utilisateur dit explicitement oui a la proposition d'e-mail.",
            DE: "Sendet E-Mail mit Link zur Kundenzone. Rufen Sie diese Funktion auf, wenn der Nutzer explizit ja sagt zur E-Mail-Anfrage.",
            EN: "Sends email with link to the customer zone. Call this function when the user explicitly says yes to receiving the email."
        },
        confirmation: "Explicit",
        entities: [
            { name: "email", type: "string", description: { NL: "E-mailadres voor de link", FR: "Adresse e-mail pour le lien", DE: "E-Mail-Adresse fur den Link", EN: "Email address for the link" }, required: false }
        ],
        silent: false
    },
    transfer_to_operator: {
        description: {
            NL: "Verbindt de gebruiker door met een medewerker. Roep deze functie aan wanneer de gebruiker een medewerker vraagt, beide opties weigert, of bij technische fout.",
            FR: "Transfere l'utilisateur a un collaborateur. Appelez cette fonction lorsque l'utilisateur demande un collaborateur, refuse les deux options, ou en cas d'erreur technique.",
            DE: "Verbindet den Nutzer mit einem Mitarbeiter. Rufen Sie diese Funktion auf, wenn der Nutzer einen Mitarbeiter verlangt, beide Optionen ablehnt, oder bei technischem Fehler.",
            EN: "Connects the user to an agent. Call this function when the user asks for an agent, refuses both options, or on technical error."
        },
        confirmation: "None",
        entities: [],
        silent: false
    ...
    end_conversation: {
        description: {
            NL: "Beëindigt het gesprek. Roep aan wanneer de gebruiker tevreden is en geen verdere vragen of acties heeft. Geen bevestiging nodig.",
            FR: "Termine la conversation. Appelez lorsque l'utilisateur est satisfait et n'a plus de questions. Aucune confirmation requise.",
            DE: "Beendet das Gesprach. Rufen Sie auf wenn der Nutzer zufrieden ist. Keine Bestatigung nötig.",
            EN: "Ends the conversation. Call when the user is satisfied and has no further questions or actions. No confirmation needed."
        },
        confirmation: "None",
        entities: [],
        silent: true
...
var SCENARIO_PROMPTS = {
    full_choice: {
        NL: "Doel: help de klant kiezen tussen uitstel van betaling en een afbetalingsplan.\n" +
            ...
            "- Roep end_conversation aan zonder te vragen wanneer de klant tevreden is en geen verdere vragen heeft. Geen afscheidsvraag nodig.\n" +
        ...
        FR: "Objectif: aider le client a choisir entre un report de paiement et un plan de paiement.\n" +
            ...
            "- Appelez end_conversation sans demander quand le client est satisfait et n'a plus de questions. Pas besoin de demander s'il veut terminer.\n" +
        ...
        DE: "Ziel: dem Kunden helfen zwischen Zahlungsaufschub und Ratenzahlung zu wahlen.\n" +
            "- Fragen Sie nach der Präferenz, falls der Kunde noch nicht gewählt hat.\n" +
            ...
            "- Rufen Sie end_conversation ohne Nachfrage auf wenn der Kunde zufrieden ist. Keine Abschiedsfrage nötig.\n" +
            "WICHTIG: keine Aktion ohne explizite Bestatigung (außer end_conversation).",
        EN: "Goal: help the customer choose between a payment deferral and an installment plan.\n" +
            ...
            "- Call end_conversation without asking when the customer is satisfied and has no further questions. No goodbye confirmation needed.\n" +
    ...
    deferral_only: {
        NL: "Doel: regel uitstel van betaling als de klant dat wil.\n" +
            ...
            "- Roep end_conversation aan zonder te vragen wanneer de klant tevreden is.\n" +
            "IMPORTANT: geen actie zonder expliciete bevestiging (behalve end_conversation).",
        FR: "Objectif: organiser un report de paiement si le client le souhaite.\n" +
            ...
            "- Appelez end_conversation sans demander quand le client est satisfait.\n" +
            "IMPORTANT: aucune action sans confirmation explicite (sauf end_conversation).",
        DE: "Ziel: Zahlungsaufschub einrichten, wenn der Kunde das mochte.\n" +
            ...
            "- Rufen Sie end_conversation ohne Nachfrage auf wenn der Kunde zufrieden ist.\n" +
            "WICHTIG: keine Aktion ohne explizite Bestatigung (außer end_conversation).",
        EN: "Goal: arrange a payment deferral if the customer wants it.\n" +
            ...
            "- Call end_conversation without asking when the customer is satisfied.\n" +
            "IMPORTANT: no action without explicit confirmation (except end_conversation)."
    },
    ...
    website_referral: {
        NL: "Doel: leg uit dat de klant dit in de klantenzone kan regelen en check of dit duidelijk is.\n" +
            ...
            "- Roep end_conversation aan zonder te vragen wanneer alles duidelijk is en er geen verdere vragen zijn.",
        FR: "Objectif: expliquer que le client peut le faire via la zone client et verifier si c'est clair.\n" +
            ...
            "- Appelez end_conversation sans demander quand tout est clair et pas d'autres questions.",
        DE: "Ziel: erklaren dass der Kunde dies im Kundenbereich erledigen kann und prufen ob es klar ist.\n" +
            ...
            "- Rufen Sie end_conversation ohne Nachfrage auf wenn alles klar und keine weiteren Fragen.",
        EN: "Goal: explain the customer can handle this in the customer zone and check if it is clear.\n" +
            ...
            "- Call end_conversation without asking when everything is clear and no further questions."
    },
    ...
    info_only: {
        NL: "Doel: beantwoord vragen over de situatie en bepaal of de klant nog hulp nodig heeft.\n" +
            ...
            "- Roep end_conversation aan zonder te vragen wanneer er geen verdere vragen zijn.",
        FR: "Objectif: repondre aux questions sur la situation et verifier si le client a besoin d'aide.\n" +
            ...
            "- Appelez end_conversation sans demander quand il n'y a plus de questions.",
        DE: "Ziel: Fragen zur Situation beantworten und prufen ob der Kunde Hilfe braucht.\n" +
            ...
            "- Rufen Sie end_conversation ohne Nachfrage auf wenn keine weiteren Fragen.",
        EN: "Goal: answer questions about the situation and check if the customer needs help.\n" +
            ...
            "- Call end_conversation without asking when there are no further questions."
    },
...
function buildObjectivePrompt(scenario, lang, vars) {
    ...
    if (scenario === 'full_choice' || scenario === 'deferral_only' || scenario === 'installment_only') {
        ...
        if (facts.length) {
            var factsLabel = FACTS_LABEL[lang] || FACTS_LABEL.EN;
            prompt += '\n\n---\n' + factsLabel + ':\n- ' + facts.join('\n- ');
        }
    }
...
function buildToolDefinitions(scenario, lang) {
    ...
    for (var i = 0; i < allowed.length; i++) {
        var name = allowed[i];
        var cfg = TOOL_CONFIG[name];
        if (!cfg) {
            result.push({ name: name, description: '', confirmation: 'None', entities: [], silent: false });
            continue;
        }
        var desc = (cfg.description && cfg.description[lang]) ? cfg.description[lang] : (cfg.description ? cfg.description.EN || '' : '');
        var entities = [];
        if (cfg.entities && cfg.entities.length) {
            for (var j = 0; j < cfg.entities.length; j++) {
                var e = cfg.entities[j];
                var eDesc = (e.description && e.description[lang]) ? e.description[lang] : (e.description ? e.description.EN || '' : '');
                entities.push({ name: e.name, type: e.type || 'string', description: eDesc, required: !!e.required });
            }
        }
        result.push({
            name: name,
...
function buildAgentContext(caseNum, payload, lang) {
    ...
    return {
        ...
        toolDefinitions: buildToolDefinitions(scenario, lang),
...
function buildFallback(caseNum, lang, reason) {
    ...
    return {
        ...
        toolData: {},
        toolDefinitions: buildToolDefinitions('fallback_error', normalizeLanguage(lang)),
...
// Fixed logging guard: use standard Vocalls logDebug function
if (typeof logDebug === 'function') {
    try {
        logDebug('PaymentAssistant: ' + JSON.stringify({
            segment: segName,
...
// Last expression ensures runInContext returns agentContext (no top-level return in vm)
agentContext;
