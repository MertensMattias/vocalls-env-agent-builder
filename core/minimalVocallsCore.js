...
var vm = require('vm');
var databaseBridge = require('./databaseBridge');
...
function createSandbox(options) {
    ...
    // ============================================================================
    // DATABASE BRIDGE (Optional - connects to vocalls-database-service)
    // ============================================================================
    
    // Initialize database bridge and expose global functions if service is available
    // These functions will be undefined if database service is not running
    try {
        databaseBridge.initialize(options.databaseUrl);
        databaseBridge.exposeGlobalFunctions();
        
        // Copy database functions from global to sandbox
        if (typeof global.dbQuery !== 'undefined') {
            sandbox.dbQuery = global.dbQuery;
        }
        if (typeof global.dbQuerySingle !== 'undefined') {
            sandbox.dbQuerySingle = global.dbQuerySingle;
        }
        if (typeof global.dbExecute !== 'undefined') {
            sandbox.dbExecute = global.dbExecute;
        }
        if (typeof global.dbStoredProc !== 'undefined') {
            sandbox.dbStoredProc = global.dbStoredProc;
        }
        
        sandbox.log_debug('[Core] Database bridge initialized and functions exposed');
    } catch (error) {
        sandbox.log_warn('[Core] Database bridge not available:', error.message);
        // Continue without database - scripts can check if functions are available
    }
...
function validateConstraints(code, filename) {
    ...
    var rules = [
        ...
        },
        {
            ...
            message: 'Use log_debug(), log_warn(), log_error() or wrapper functions instead of console.*'
        }
    ];
    ...
    rules.forEach(function (rule) {
        lines.forEach(function (line, lineIndex) {
            ...
            while ((match = rule.pattern.exec(line)) !== null) {
                var snippet = line.substring(Math.max(0, match.index - 10), match.index + match[0].length + 10);
                errors.push({
                    rule: rule.name,
                    line: lineIndex + 1,
                    col: match.index + 1,
