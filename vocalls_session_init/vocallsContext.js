...
/**
 * Loads environment configuration from env.config.json
 *
 * @param {string} projectRoot - Path to the project root
 * @returns {Object} Configuration object
 */
function loadEnvConfig(projectRoot) {
    var defaultConfig = {
        httpMode: 'real',
        storageMode: 'disk',
        env: 'acc',
    ...
    try {
        var configPath = path.join(projectRoot || process.cwd(), 'env.config.json');
        ...
        console.warn('Failed to load env.config.json:', error.message);
    }
...
function buildSessionContext(seed, options) {
    ...
    var resolvedHttpMode = options.httpMode || projectSettings.httpMode || envConfig.httpMode || 'real';
    var resolvedStorageMode = options.storageMode || projectSettings.storageMode || envConfig.storageMode || 'disk';
    var resolvedDatabaseUrl = options.databaseUrl || projectSettings.databaseUrl || envConfig.databaseUrl || DEFAULT_DATABASE_URL;

    // Create the core sandbox
    var sandbox = core.createSandbox({
    ...
    if (resolvedDatabaseUrl) {
        sandbox.context.settings.databaseUrl = resolvedDatabaseUrl;
    }
...
function createDefaultSeed(scenario, overrides) {
    ...
    var envConfig = loadEnvConfig();
    var moduleName = (overrides.settings && overrides.settings.moduleName) || envConfig.moduleName || 'vocalls-env';
    var defaultEnvironment = (overrides.settings && overrides.settings.defaultEnvironment) || envConfig.env || 'acc';
    ...
    if (!seed) {
        throw new Error('Unknown scenario: ' + scenario + '. Available: ' + Object.keys(seeds).join(', '));
    }
